...
func (this *RaftNode) HandleRequestVote(args RequestVoteArgs, reply *RequestVoteReply) error {
	...
	if args.Term == this.currentTerm && (this.votedFor == -1 || this.votedFor == args.CandidateId) &&
		(nodeLastLogTerm < args.LastLogTerm ||
			(nodeLastLogTerm == args.LastLogTerm && nodeLastLogIndex <= args.LastLogIndex)) { // TODO: what are the conditions necessary to vote? HINT: there's multiple.
